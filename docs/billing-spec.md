...
運用ルール
- ユーザーは Cognito `sub` を永続キーとする。メールアドレスはDB主キーに使わない。
- 管理者/メンバー/フォルダ招待メンバーは別ユーザー種別ではなく、部屋ごとの `role` / `folderScope` / `folderIds` で表現する。
- 1つのお部屋内では、ユーザーの立場は `管理者` / `お部屋メンバー` / `フォルダメンバー` のどれか1つだけとする。
- 管理者は原則「部屋（チーム）を作成したユーザー」
- 入室したユーザーは自動で `role=member` になる
- お部屋招待メンバーは `folderScope=all`、フォルダ招待メンバーは `folderScope=invited` + `folderIds` で閲覧範囲を制御する
- お部屋メンバーはお部屋管理でだけ削除し、フォルダメンバーは対象フォルダの管理でだけ外す。
- お部屋メンバーがフォルダ招待を受けてもフォルダメンバーには降格しない。フォルダメンバーが お部屋招待を受けた場合はお部屋メンバーへ昇格する。
- メンバー削除は物理削除ではなく `status=left`。再招待された場合は `active` に戻して再参加できる
- 1ユーザーが作れる部屋は1つまで
