...
function demoNow(minutesAgo = 0) {
  return new Date(Date.now() - minutesAgo * 60 * 1000).toISOString();
}

function demoSvg(label, toneA, toneB) {
  const svg = `
...
uploadFiles = async function () {
  ...
  try {
    ...
    drafts.forEach((draft) => {
      ...
      const fileName = String(draft.photoName || draft.originalName || `写真${demoStore.photoCounter}`).trim();
      const viewUrl = draft.file ? URL.createObjectURL(draft.file) : demoSvg(fileName, '#7baf6a', '#dff0d8');
...
api = async function (path, options = {}) {
  ...
  if (path.startsWith('/folders/') && path.endsWith('/export') && method === 'POST') {
    const roomName = activeRoom?.roomName || 'デモルーム';
    const folderId = path.split('/')[2];
    const { folder } = demoFindFolder(folderId);
    const content = `${roomName}\n${folder?.title || 'フォルダ'}\n出力プレビューです。`;
    return { downloadUrl: `data:text/plain;charset=utf-8,${encodeURIComponent(content)}` };
  }
  ...
  if (path.startsWith('/photos/') && method === 'PUT') {
    ...
    const { photo } = demoFindPhoto(photoId);
    if (!photo) throw demoError(404, 'photo not found');
    photo.fileName = String(body.fileName || photo.fileName).trim() || photo.fileName;
    return demoPhotoSummary(photo);
  }
